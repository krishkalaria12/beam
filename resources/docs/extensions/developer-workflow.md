...
The generated extension is based on [`resources/extra/extension-boilerplate`](../../extra/extension-boilerplate).
...
Beam’s store publisher expects a source directory under `infra/store/packages/` containing:

- `package.json`: the extension manifest
- `beam-store.json`: store metadata, release metadata, compatibility, and verification details

Use an existing source package in `infra/store/packages/` as the reference shape.
...
- release zip artifacts in `infra/store/artifacts/`
- checksums
- updated catalog entries in `infra/store/catalog.json`
