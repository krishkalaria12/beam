// PORT: apps/desktop/src-tauri/build.rs (proto compilation only)
// Compiles the extension-runtime protobuf definitions into OUT_DIR. The
// SoulverWrapper rpath patching stays with the packaging lane (G3) — it is
// a build artifact step, not a proto step.
...
fn compile_extension_runtime_proto() -> Result<(), Box<dyn Error>> {
    ...
    config.compile_protos(&proto_paths, &[proto_dir.as_path()])?;

    Ok(())
}

fn main() {
    if let Err(error) = compile_extension_runtime_proto() {
        println!("cargo:warning=extension-runtime proto compilation failed: {error}");
        std::process::exit(1);
    }
}
