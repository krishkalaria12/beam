// PORT: apps/desktop/src-tauri/src/todo/mod.rs
...
pub async fn get_todo(cx: &BeamContext, todo_id: String) -> Result<TodoWithSubTodos> {
...
pub async fn get_todos(cx: &BeamContext) -> Result<Vec<TodoWithSubTodos>> {
    let pool: TodoDbPool = get_todo_pool(cx).await?;
