// PORT: apps/desktop/src-tauri/src/snippets/db.rs
...
pub fn init(cx: &BeamContext) {
    ...
    tokio::spawn(async move {
        if let Err(error) = get_snippets_pool(&context).await {
            log::error!("snippets sqlite initialization failed: {error}");
        }
    });
}

pub async fn init_snippets_db(cx: &BeamContext) -> Result<()> {
    let _ = get_snippets_pool(cx).await?;
    Ok(())
}

pub async fn get_snippets_pool(cx: &BeamContext) -> Result<SnippetDbPool> {
    let context = cx.clone();

    let pool = SNIPPET_POOL
        .get_or_try_init(|| async move {
            let database_path = get_snippets_database_path(&context)?;
...
pub fn get_snippets_database_path(cx: &BeamContext) -> Result<PathBuf> {
