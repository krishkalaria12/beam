// PORT: apps/desktop/src-tauri/src/snippets/runtime.rs
...
static APP_SNIPPETS: std::sync::OnceLock<std::sync::Arc<SnippetsState>> =
...
pub async fn refresh_runtime_state(cx: &BeamContext) -> Result<()> {
    let repository = SnippetsRepository::new();
    let settings = repository.get_runtime_settings(cx).await?;
    let snippets = repository.list_snippets(cx).await?;
    let state = APP_SNIPPETS.get().cloned().ok_or_else(|| {
        SnippetError::ValidationError("snippets runtime is not initialized".to_string())
    })?;
...
pub fn initialize_runtime(cx: &BeamContext, snippets: std::sync::Arc<SnippetsState>) {
    let _ = APP_SNIPPETS.set(snippets);
    let state = APP_SNIPPETS
        .get()
        .cloned()
        .expect("snippets state was just installed");
    let _ = state.status_tx.send(RuntimeStatus::Starting);

    let init_cx = cx.clone();
    tokio::spawn(async move {
        if let Err(error) = refresh_runtime_state(&init_cx).await {
            let _ = state
                .status_tx
    ...
    #[cfg(target_os = "linux")]
    start_linux_runtime_once(cx.clone());
}
...
pub async fn paste_snippet(cx: &BeamContext, snippet_id: &str) -> Result<()> {
    let repository = SnippetsRepository::new();
    let snippet = repository
        .get_snippet_by_id(cx, snippet_id)
        .await?
    ...
    repository
        .record_snippet_usage(cx, &snippet.id, true)
        .await?;
    Ok(())
}

#[cfg(target_os = "linux")]
fn start_linux_runtime_once(cx: BeamContext) {
    ...
    tokio::spawn_blocking(move || run_linux_runtime(cx));
}

#[cfg(target_os = "linux")]
fn run_linux_runtime(cx: BeamContext) {
    let state = APP_SNIPPETS.get().cloned().ok_or_else(|| {
        SnippetError::ValidationError("snippets runtime is not initialized".to_string())
    })?;
    let mut runtime = match LinuxSnippetRuntime::new(cx) {
        Ok(runtime) => runtime,
        Err(error) => {
...
impl LinuxSnippetRuntime {
    fn new(cx: BeamContext) -> Result<Self> {
    ...
    fn find_matching_snippet(&self, instant: bool) -> Option<Snippet> {
        let snippets_state = self.APP_SNIPPETS.get().cloned().ok_or_else(|| {
            SnippetError::ValidationError("snippets runtime is not initialized".to_string())
        })?;
    ...
    fn expand_snippet(
        ...
        tauri::async_runtime::block_on(async {
            SnippetsRepository::new()
                .record_snippet_usage(&self.cx, &snippet.id, true)
                .await
        })?;
