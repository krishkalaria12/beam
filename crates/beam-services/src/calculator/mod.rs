...
// PORT: apps/desktop/src-tauri/src/calculator/mod.rs
// Command attributes deleted; names and argument order preserved.
...
/// Soulver runs through a Swift FFI built only for Linux; the wrapper lives
/// under the bundled resource directory.
#[cfg(target_os = "linux")]
pub fn initialize(cx: &BeamContext) -> Result<()> {
...
#[cfg(not(target_os = "linux"))]
pub fn initialize(_cx: &BeamContext) -> Result<()> {
    Ok(())
}
...
pub async fn get_calculator_history(cx: &BeamContext) -> Result<Vec<CalculatorHistoryEntry>> {
...
pub async fn save_calculator_history(
    ...
    save_to_history(cx, query, result, session_id).await
}

pub async fn delete_calculator_history_entry(cx: &BeamContext, timestamp: i64) -> Result<()> {
    delete_history_entry(cx, timestamp).await
}

pub async fn clear_calculator_history(cx: &BeamContext) -> Result<()> {
    clear_history(cx).await
}

pub async fn get_pinned_calculator_history_timestamps(cx: &BeamContext) -> Result<Vec<i64>> {
    get_pinned_timestamps(cx).await
}
...
pub async fn set_calculator_history_entry_pinned(
    ...
    set_history_entry_pinned(cx, timestamp, pinned).await
}
