...
// PORT: apps/desktop/src-tauri/src/calculator/history.rs
// AppHandle replaced by BeamContext.
...
pub async fn get_history(cx: &BeamContext) -> Result<Vec<CalculatorHistoryEntry>> {
    let pool = get_calculator_pool(cx).await?;
...
pub async fn delete_history_entry(cx: &BeamContext, timestamp: i64) -> Result<()> {
    let _guard = CALCULATOR_HISTORY_WRITE_LOCK.lock().await;
    let pool = get_calculator_pool(cx).await?;
...
pub async fn get_pinned_timestamps(cx: &BeamContext) -> Result<Vec<i64>> {
    let pool = get_calculator_pool(cx).await?;
