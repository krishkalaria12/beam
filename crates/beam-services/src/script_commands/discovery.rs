// PORT: apps/desktop/src-tauri/src/script_commands/discovery.rs
...
pub(super) fn resolve_script_commands_directory(cx: &BeamContext) -> Result<PathBuf> {
    let base_directory = cx.paths().local_data_dir().clone();
