// PORT: apps/desktop/src-tauri/src/script_commands/runner.rs
...
fn resolve_target_command(cx: &BeamContext, command_id: &str) -> Result<ScriptCommandSummary> {
    let commands = cache::get_script_commands(cx)?;
    ...
    let fresh_commands = cache::get_script_commands(cx)?;
    fresh_commands
        .into_iter()
        .find(|command| command.id == command_id)
...
fn ensure_script_path_is_within_root(cx: &BeamContext, script_path: &Path) -> Result<()> {
    let root = resolve_script_commands_directory(cx)?;
