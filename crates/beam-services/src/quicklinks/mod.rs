...
// PORT: apps/desktop/src-tauri/src/quicklinks/mod.rs
// Command attributes deleted; AppHandle became &BeamContext. The Window
// parameter on execute_quicklink went away with the webview: the service
// opens the target, the dispatcher hides the launcher.
...
pub fn create_quicklink(cx: &BeamContext, quick_link_data: Quicklink) -> Result<()> {
    quick_link_data.is_valid()?;

    let existing_ql = get_quicklinks_from_store(cx)?;
    ...
    save_quicklinks_to_store(cx, &quick_link_data)?;

    Ok(())
}

pub fn delete_quicklink(cx: &BeamContext, keyword: String) -> Result<()> {
...
pub fn get_quicklinks(cx: &BeamContext) -> Result<Vec<Quicklink>> {
    get_quicklinks_from_store(cx)
}

pub fn update_quicklink(cx: &BeamContext, keyword: String, new_quicklink: Quicklink) -> Result<()> {
    let keyword = keyword.trim();
...
pub fn execute_quicklink(cx: &BeamContext, keyword: String, query: String) -> Result<()> {
    let keyword = keyword.trim();
    ...
    let quick_links = get_quicklinks_from_store(cx)?;
    ...
    // Hiding the launcher after execution is the dispatcher's job now.
