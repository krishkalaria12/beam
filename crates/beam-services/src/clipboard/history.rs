// PORT: apps/desktop/src-tauri/src/clipboard/history.rs
...
pub async fn get_history(cx: &BeamContext) -> Result<Vec<ClipboardHistoryEntry>> {
    let pool = get_clipboard_pool(cx).await?;
    let (history, _) = load_visible_history(pool.as_ref()).await?;

    Ok(history.into_iter().map(|record| record.entry).collect())
}

pub async fn get_history_values(cx: &BeamContext) -> Result<Vec<String>> {
    let history = get_history(cx).await?;
    Ok(history.into_iter().map(|entry| entry.value).collect())
}

pub async fn save_to_history(cx: &BeamContext, copy_value: String) -> Result<()> {
...
pub async fn clear_history(cx: &BeamContext) -> Result<()> {
    let _guard = CLIPBOARD_HISTORY_WRITE_LOCK.lock().await;
    let pool = get_clipboard_pool(cx).await?;
...
pub async fn get_pinned_entry_ids(cx: &BeamContext) -> Result<Vec<String>> {
    let pool = get_clipboard_pool(cx).await?;
