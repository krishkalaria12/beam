// PORT: apps/desktop/src-tauri/src/danksearch/bootstrap.rs
#[cfg(target_os = "linux")]
...
#[cfg(target_os = "linux")]
pub fn initialize(cx: &BeamContext) {
    tokio::spawn(async move {
        let paths = match config::ensure_config(cx) {
            Ok(paths) => paths,
            Err(error) => {
...
pub fn initialize(_cx: &BeamContext) {}
