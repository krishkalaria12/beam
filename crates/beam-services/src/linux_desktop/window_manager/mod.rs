// PORT: apps/desktop/src-tauri/src/linux_desktop/window_manager/mod.rs
...
pub fn list_windows(cx: &BeamContext, state: &AppState) -> Result<Vec<WindowEntry>> {
    let env = detect_environment();
    let selected_icon_theme = crate::settings::get_selected_icon_theme(cx).ok().flatten();
