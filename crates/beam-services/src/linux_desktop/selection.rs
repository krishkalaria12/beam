// PORT: apps/desktop/src-tauri/src/linux_desktop/selection.rs
...
fn open_connection() -> Result<(RustConnection, usize, SelectionAtoms)> {
    let (connection, screen_num) = x11rb::connect(None).map_err(|error| {
        LinuxDesktopError::SelectedTextError(format!("failed to connect to X11: {error}"))
    })?;
    let atoms = SelectionAtoms::new(&connection)
        .map_err(|error| {
            LinuxDesktopError::SelectedTextError(format!(
                "failed to request X11 selection atoms: {error}"
            ))
        })?
        .reply()
        .map_err(|error| {
            LinuxDesktopError::SelectedTextError(format!(
                "failed to read X11 selection atoms: {error}"
            ))
        })?;
...
fn request_selection(
    ...
    let window = connection.generate_id().map_err(|error| {
        LinuxDesktopError::SelectedTextError(format!("failed to allocate X11 window id: {error}"))
    })?;
    ...
    connection
        ...
        .map_err(|error| {
            LinuxDesktopError::SelectedTextError(format!(
                "failed to create X11 selection window: {error}"
            ))
        })?;
    ...
    connection
        ...
        .map_err(|error| {
            LinuxDesktopError::SelectedTextError(format!(
                "failed to request X11 PRIMARY selection: {error}"
            ))
        })?;
    connection.flush().map_err(|error| {
        LinuxDesktopError::SelectedTextError(format!(
            "failed to flush X11 selection request: {error}"
        ))
    })?;

    let deadline = Instant::now() + Duration::from_millis(800);
    let result = loop {
        if let Some(event) = connection.poll_for_event().map_err(|error| {
            LinuxDesktopError::SelectedTextError(format!("failed to poll X11 events: {error}"))
        })? {
            match event {
                Event::SelectionNotify(event) if event.requestor == window => {
                    if event.property == NONE {
                        break Err(LinuxDesktopError::SelectedTextError(
                            "PRIMARY selection did not provide text data".to_string(),
                        ));
                    }
                    ...
                    let reply = connection
                        ...
                        .map_err(|error| {
                            LinuxDesktopError::SelectedTextError(format!(
                                "failed to fetch X11 selection property: {error}"
                            ))
                        })?
                        .reply()
                        .map_err(|error| {
                            LinuxDesktopError::SelectedTextError(format!(
                                "failed to read X11 selection property: {error}"
                            ))
                        })?;
        ...
        if Instant::now() >= deadline {
            break Err(LinuxDesktopError::SelectedTextError(
                "timed out while waiting for X11 PRIMARY selection".to_string(),
            ));
        }
...
pub fn read_x11_primary_selection() -> Result<String> {
    ...
    Err(LinuxDesktopError::SelectedTextError(
        "PRIMARY selection is unavailable on this X11 session".to_string(),
    ))
}
