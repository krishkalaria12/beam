// PORT: apps/desktop/src-tauri/src/ai/helper.rs
// AppHandle became &BeamContext; stream events ride the typed bus.
...
fn get_settings_path(cx: &BeamContext) -> Result<PathBuf> {
...
fn emit_stream_chunk(cx: &BeamContext, request_id: &str, text: &str) -> Result<()> {
    cx.emit(BeamEvent::AiStreamChunk(beam_core::events::AiStreamChunk {
...
fn emit_stream_end(cx: &BeamContext, request_id: &str, full_text: &str) -> Result<()> {
    cx.emit(BeamEvent::AiStreamEnd(beam_core::events::AiStreamEnd {
        request_id: request_id.to_string(),
...
fn emit_stream_error(cx: &BeamContext, request_id: &str, error: &str) -> Result<()> {
    cx.emit(BeamEvent::AiStreamError(beam_core::events::AiStreamError {
        request_id: request_id.to_string(),
...
pub fn get_ai_settings(cx: &BeamContext) -> Result<AiSettings> {
    let path = get_settings_path(cx)?;
    read_settings(&path)
}

pub fn set_ai_settings(cx: &BeamContext, settings: AiSettings) -> Result<()> {
    let path = get_settings_path(cx)?;
...
pub async fn get_ai_chat_history(
    ...
    let messages = repository
        .get_chat_history(cx, conversation_id.as_deref(), limit)
        .await?;
    ...
    let attachments = repository
        .get_message_attachments(cx, conversation_id.as_deref())
        .await?;
    ...
    for attachment in attachments {
        ...
        if let Some(data_url) =
            load_attachment_data_url(cx, &attachment.storage_path, &attachment.mime_type)
        {
...
fn load_attachment_data_url(
    ...
    let app_data_dir = cx.paths().local_data_dir().clone();
...
pub async fn get_ai_conversations(
    ...
    AiRepository::new().get_conversations(cx, limit).await
}
...
pub async fn clear_ai_chat_history(
    ...
    AiRepository::new()
        .clear_chat_history(cx, conversation_id.as_deref())
        .await
}
...
pub async fn get_ai_token_usage_summary(
    ...
    AiRepository::new()
        .get_token_usage_summary(cx, conversation_id.as_deref())
        .await
}
...
pub fn ai_can_access(cx: &BeamContext, provider_id: Option<String>) -> Result<bool> {
    let settings = get_ai_settings(cx)?;
    if !settings.enabled {
        return Ok(false);
    }
...
pub async fn ai_ask_stream(
    ...
    let settings = get_ai_settings(&cx)?;
    if !settings.enabled {
        emit_stream_error(&cx, &request_id, &AiError::AccessDisabled.to_string())?;
        return Ok(());
    }
    ...
    let api_key = match get_provider_api_key(provider) {
        Ok(value) => value,
        Err(error) => {
            emit_stream_error(&cx, &request_id, &error.to_string())?;
            return Ok(());
        }
    };
    ...
    let mut context_preparation = match context_manager
        .prepare_context_prompt(&cx, provider, options.conversation_id.as_deref(), &prompt)
        .await
    {
        Ok(value) => value,
        Err(error) => {
    ...
    let stream_result =
        run_provider_completion(provider, &api_key, &model_name, prompt_message, |text| {
            emit_stream_chunk(&cx, &request_id, text)
        })
        .await;
    ...
    emit_stream_end(&cx, &request_id, &stream_outcome.full_text)?;
