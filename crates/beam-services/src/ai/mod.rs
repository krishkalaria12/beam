// PORT: apps/desktop/src-tauri/src/ai/mod.rs
use beam_core::BeamContext;

pub mod attachments;
pub(crate) mod config;
pub mod context;
pub mod db;
pub mod error;
pub mod helper;
pub mod key_store;
pub mod model;
pub mod repository;

pub use error::{AiError, Result};
pub use model::{
    AiChatHistoryMessage, AiConversationSummary, AiPersistedMessage, AiSettings,
    AiTokenUsageSummary, AskOptions,
};

#[allow(non_snake_case)]
fn resolve_provider_arg(provider_id: Option<String>, providerId: Option<String>) -> Option<String> {
    provider_id.or(providerId)
}

pub fn get_ai_settings(cx: &BeamContext) -> Result<AiSettings> {
    helper::get_ai_settings(cx)
}

pub fn set_ai_settings(cx: &BeamContext, settings: AiSettings) -> Result<()> {
    helper::set_ai_settings(cx, settings)
}

#[allow(non_snake_case)]
pub async fn get_ai_chat_history(
    cx: &BeamContext,
    conversation_id: Option<String>,
    conversationId: Option<String>,
    limit: Option<u32>,
) -> Result<Vec<AiChatHistoryMessage>> {
    helper::get_ai_chat_history(cx, conversation_id.or(conversationId), limit).await
}

pub async fn get_ai_conversations(
    cx: &BeamContext,
    limit: Option<u32>,
) -> Result<Vec<AiConversationSummary>> {
    helper::get_ai_conversations(cx, limit).await
}

#[allow(non_snake_case)]
pub async fn clear_ai_chat_history(
    cx: &BeamContext,
    conversation_id: Option<String>,
    conversationId: Option<String>,
) -> Result<()> {
    helper::clear_ai_chat_history(cx, conversation_id.or(conversationId)).await
}

#[allow(non_snake_case)]
pub async fn get_ai_token_usage_summary(
    cx: &BeamContext,
    conversation_id: Option<String>,
    conversationId: Option<String>,
) -> Result<AiTokenUsageSummary> {
    helper::get_ai_token_usage_summary(cx, conversation_id.or(conversationId)).await
}

#[allow(non_snake_case)]
pub fn set_ai_api_key(
    key: String,
    provider_id: Option<String>,
    providerId: Option<String>,
) -> Result<()> {
    helper::set_ai_api_key(key, resolve_provider_arg(provider_id, providerId))
}

#[allow(non_snake_case)]
pub fn is_ai_api_key_set(provider_id: Option<String>, providerId: Option<String>) -> Result<bool> {
    helper::is_ai_api_key_set(resolve_provider_arg(provider_id, providerId))
}

#[allow(non_snake_case)]
pub fn clear_ai_api_key(provider_id: Option<String>, providerId: Option<String>) -> Result<()> {
    helper::clear_ai_api_key(resolve_provider_arg(provider_id, providerId))
}

pub fn ai_can_access(cx: &BeamContext, provider_id: Option<String>) -> Result<bool> {
    helper::ai_can_access(cx, provider_id)
}

pub async fn ai_ask_stream(
    cx: &BeamContext,
    request_id: String,
    prompt: String,
    options: AskOptions,
) -> Result<()> {
    helper::ai_ask_stream(cx, request_id, prompt, options).await
}
