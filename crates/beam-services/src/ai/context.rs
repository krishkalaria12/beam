// PORT: apps/desktop/src-tauri/src/ai/context.rs
...
impl ContextManager {
    ...
    pub async fn prepare_context_prompt(
        ...
        let context_state = self
            .repository
            .get_context_state(cx, Some(&normalized_conversation_id))
            .await?;
        ...
        let total_tokens_so_far = self
            .repository
            .get_total_tokens(cx, Some(&normalized_conversation_id))
            .await?
            .max(0) as u64;
    ...
    pub async fn persist_compaction_summary(
        ...
        let current_total_tokens = self
            .repository
            .get_total_tokens(cx, Some(conversation_id))
            .await?;
