// PORT: apps/desktop/src-tauri/src/ai/repository.rs
use std::fs;
...
impl AiRepository {
    ...
    pub async fn get_chat_history(
        ...
        let pool: AiDbPool = get_ai_pool(cx).await?;
    ...
    pub async fn clear_chat_history(
        ...
        for (relative_path,) in attachment_paths {
            let _ = remove_attachment_file(cx, &relative_path);
        }
...
fn prepare_attachment_row(
    ...
    let absolute_path = resolve_storage_absolute_path(cx, &storage_path)?;
...
fn resolve_storage_absolute_path(cx: &BeamContext, relative_path: &str) -> Result<PathBuf> {
    Ok(cx.paths().local_data_dir().join(relative_path))
}

fn remove_attachment_file(cx: &BeamContext, relative_path: &str) -> Result<()> {
    let absolute_path = resolve_storage_absolute_path(cx, relative_path)?;
