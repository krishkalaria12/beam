// PORT: apps/desktop/src-tauri/src/notes/mod.rs
...
pub async fn get_notes(cx: &BeamContext) -> Result<Vec<Note>> {
    let pool: NotesDbPool = get_notes_pool(cx).await?;
...
pub async fn create_note(cx: &BeamContext, payload: CreateNotePayload) -> Result<Note> {
...
pub async fn update_note(cx: &BeamContext, payload: UpdateNotePayload) -> Result<Note> {
...
pub async fn delete_note(cx: &BeamContext, note_id: String) -> Result<()> {
