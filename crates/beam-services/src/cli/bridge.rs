...
// PORT: apps/desktop/src-tauri/src/cli/bridge.rs
// AppHandle became BeamContext; the webview window pokes (visibility check,
// reveal) became injected UI hooks owned by the binary — services never see
// a window. The dmenu event rides the typed bus instead of IPC.

use crate::cli::config::CONFIG as CLI_CONFIG;
use crate::cli::dmenu::{rank_rows, DmenuOptions, DmenuRequest, DmenuResponse};
use crate::cli::error::{CliError, Result};

/// The two window operations the bridge needs, injected by the binary at
/// startup. Everything else about the launcher surface is invisible here.
#[derive(Clone)]
pub struct BridgeUiHooks {
    pub is_launcher_visible: std::sync::Arc<dyn Fn() -> bool + Send + Sync>,
...
impl CliBridgeRuntime {
    pub fn new() -> Self {
        Self {
            ...
            hooks: BridgeHooksCell::default(),
        }
    }

    /// Installs the UI hooks once the binary has a window to show. Until
    /// then the bridge serves health checks only.
    pub fn install_hooks(&self, hooks: BridgeUiHooks) {
    ...
    pub fn enqueue_request(
        ...
        let (tx, rx) = bounded::<DmenuResponse>(1);
        {
            ...
            request.restore_window_hidden = (self.hooks.get().is_launcher_visible)();
        ...
        self.dispatch_next_request(cx);
        rx.recv().map_err(|_| CliError::RequestChannelClosed)
    }

    pub fn complete_request(&self, cx: &BeamContext, response: DmenuResponse) -> Result<()> {
        let responder = {
            let mut state = self.state.lock();
    ...
    fn dispatch_next_request(&self, cx: &BeamContext) {
        let next_request = {
            let mut state = self.state.lock();
        ...
        (self.hooks.get().show_launcher)();
        cx.emit(BeamEvent::CliDmenuRequest(
            serde_json::to_value(&next_request).unwrap_or(Value::Null),
        ));
    }
}

/// Hooks are installed right after construction; until then the bridge is
/// inert (the HTTP server only reports health).
#[derive(Default, Clone)]
pub struct BridgeHooksCell {
    cell: std::sync::Arc<Mutex<Option<BridgeUiHooks>>>,
}

impl BridgeHooksCell {
    pub fn set(&self, hooks: BridgeUiHooks) {
        *self.cell.lock() = Some(hooks);
    }

    fn get(&self) -> BridgeUiHooks {
        self.cell.lock().clone().unwrap_or_else(|| BridgeUiHooks {
            is_launcher_visible: std::sync::Arc::new(|| false),
            show_launcher: std::sync::Arc::new(|| {}),
        })
    }
}

impl Default for CliBridgeRuntime {
    fn default() -> Self {
        Self::new()
    }
}
...
struct BridgeServerState {
    context: BeamContext,
    runtime: std::sync::Arc<CliBridgeRuntime>,
}

fn handle_bridge_http_request(state: &BridgeServerState, mut request: Request) {
    ...
    match (method, path) {
        (Method::Get, "/cli/health") => {
            send_json_response(
                ...
                json!(CliBridgeHealth {
                    ok: true,
                    ui_ready: state.runtime.ui_ready(),
                }),
            );
        }
        (Method::Post, "/cli/dmenu") => {
            match parse_json_body::<DmenuBridgeSubmission>(&mut request) {
                Ok(payload) => {
                    match state.runtime.enqueue_request(
                        &state.context,
                        payload.request,
                        payload.options,
                    ) {
...
fn run_cli_bridge_server(state: BridgeServerState) -> Result<()> {
    ...
    for request in server.incoming_requests() {
        handle_bridge_http_request(&state, request);
    }

    Ok(())
}

/// Starts the CLI bridge HTTP server. `runtime` is the shared instance from
/// [`crate::state::AppState`]; attach UI hooks with
/// [`CliBridgeRuntime::install_hooks`] once the window exists.
pub fn start_cli_bridge_server(cx: &BeamContext, runtime: std::sync::Arc<CliBridgeRuntime>) {
    ...
    std::thread::spawn(move || {
        if let Err(error) = run_cli_bridge_server(state) {
...
pub fn cli_bridge_mark_ui_ready(state: &crate::state::AppState) {
    state.cli_bridge.mark_ui_ready();
}
