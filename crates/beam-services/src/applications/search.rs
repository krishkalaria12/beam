// PORT: apps/desktop/src-tauri/src/applications/search.rs
...
pub fn search_applications(cx: &BeamContext, query: String) -> Result<Vec<AppEntry>> {
    let normalized_query = query.trim();
    if normalized_query.is_empty() {
        return Ok(Vec::new());
    }

    let applications = get_searchable_applications(cx)?;
    Ok(
