...
// PORT: apps/desktop/src-tauri/src/extensions/runtime/bridge.rs
// AppHandle became &BeamContext; runtime events ride the typed bus. The
// extension-manager entry resolution keeps the repo-layout fallbacks and
// adds the resource dir the window module ships.
...
fn resolve_extension_manager_entry(cx: &BeamContext) -> Result<PathBuf, String> {
    let mut candidate_dirs = Vec::new();

    let resource_dir = cx.paths().resource_dir();
...
fn build_extension_manager_args(cx: &BeamContext) -> Result<Vec<String>, String> {
    let data_dir = cx.paths().local_data_dir().clone();
    let cache_dir = cx.paths().cache_dir().clone();
...
fn resolve_extension_runtime_launcher(
    cx: &BeamContext,
) -> Result<ExtensionRuntimeLauncher, String> {
    let args = build_extension_manager_args(cx)?;
    let entry = resolve_extension_manager_entry(cx)?;
...
fn emit_runtime_message(cx: &BeamContext, runtime_id: &str, value: &Value) {
    cx.emit(BeamEvent::ExtensionRuntimeMessage(json!({
        "runtimeId": runtime_id,
        "message": value,
    })));
}

fn emit_runtime_stderr(cx: &BeamContext, runtime_id: &str, line: &str) {
    cx.emit(BeamEvent::ExtensionRuntimeStderr(json!({
        "runtimeId": runtime_id,
        "line": line,
    })));
}

fn emit_runtime_exit(cx: &BeamContext, runtime_id: &str) {
    cx.emit(BeamEvent::ExtensionRuntimeExit(json!({
        "runtimeId": runtime_id,
    })));
}
...
fn spawn_stdout_reader(
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            emit_runtime_message(&cx, &runtime_id, &decoded_value);
        }
        ...
        emit_runtime_exit(&cx, &runtime_id);
    });
}

fn spawn_stderr_reader(cx: BeamContext, runtime_id: String, stderr: ChildStderr) {
    std::thread::spawn(move || {
        let reader = BufReader::new(stderr);
        for line in reader.lines().map_while(Result::ok) {
            if !line.trim().is_empty() {
                emit_runtime_stderr(&cx, &runtime_id, &line);
            }
        }
    });
}
...
impl ExtensionRuntimeBridgeState {
    fn ensure_started(&self, cx: &BeamContext, runtime_id: &str) -> Result<(), String> {
        ...
        let launcher = resolve_extension_runtime_launcher(cx)?;
        ...
        spawn_stderr_reader(cx.clone(), runtime_id.to_string(), stderr);
    ...
    fn send_wire_message(
        ...
        self.ensure_started(cx, runtime_id)?;
...
pub fn extension_runtime_start(
    cx: BeamContext,
    state: &ExtensionRuntimeBridgeState,
    runtime_id: Option<String>,
) -> Result<bool, String> {
    let runtime_id = normalize_runtime_id(runtime_id);
    state.ensure_started(&cx, &runtime_id)?;
    Ok(true)
}
...
pub fn extension_runtime_send_manager_request(
    ...
    if let Err(error) = state.send_wire_message(&cx, &runtime_id, &wire_message) {
