// PORT: apps/desktop/src-tauri/src/extensions/store.rs
...
fn resolve_catalog_file_candidates(cx: &BeamContext) -> Vec<PathBuf> {
...
async fn load_catalog(cx: &BeamContext) -> Result<proto::ExtensionStoreCatalog> {
    ...
    for candidate in resolve_catalog_file_candidates(cx) {
    ...
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let default_base = find_repo_root(&manifest_dir).map(|repo_root| {
        CatalogBase::File(repo_root.join(EXTENSIONS_CONFIG.store_catalog.directory_name))
    });
...
pub(crate) async fn resolve_store_artifact(
    ...
    let package = if let Some((owner, slug)) = parse_raycast_package_id(normalized_package_id) {
        ...
        let catalog = load_catalog(cx).await?;
...
pub async fn get_extension_store_updates(cx: &BeamContext) -> Result<JsonValue> {
    let catalog = load_catalog(cx).await?;
    let discovered = discover_plugins(cx)?;
...
mod tests {
    ...
    fn catalog_fixture_parses_into_publishable_packages() {
        let catalog = load_catalog_from_str(
            include_str!("../../../../infra/store/catalog.json"),
            Some(&CatalogBase::File(PathBuf::from("infra/store"))),
        )
