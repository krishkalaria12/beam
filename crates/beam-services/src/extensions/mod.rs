// PORT: apps/desktop/src-tauri/src/extensions/mod.rs
...
fn get_extension_dir(cx: &BeamContext, slug: &str) -> Result<PathBuf> {
...
fn install_extension_archive(
    ...
    let extension_dir = get_extension_dir(cx, slug)?;
...
pub fn discover_plugins(cx: &BeamContext) -> Result<Vec<proto::DiscoveredPlugin>> {
    let plugins_base_dir = get_extension_dir(cx, "")?;
...
pub fn get_discovered_plugins(cx: &BeamContext) -> Result<Vec<JsonValue>> {
    discover_plugins(cx).map(|plugins| {
        plugins
            .iter()
...
pub async fn install_store_extension(
    ...
    let install_result = install_extension_archive(cx, &resolved.package.slug, &content, force)?;
    if matches!(install_result, InstallResult::Success) {
        sync_installed_store_manifest(cx, &resolved.package.slug, &resolved.package)?;
    }

    Ok(install_result)
}

pub fn uninstall_extension(cx: &BeamContext, slug: String) -> Result<bool> {
    if !is_valid_extension_slug(&slug) {
