// PORT: apps/desktop/src-tauri/src/extensions/browser_extension.rs
// The tauri AppHandle became a BeamContext handle; the bridge's HTTP
// server is unchanged.
...
static BRIDGE_APP_HANDLE: OnceLock<BeamContext> = OnceLock::new();
    ...
    match (method, path) {
        ...
        (Method::Post, "/bridge/focus/snooze") => {
            match parse_json_body::<BridgeFocusSnoozePayload>(&mut request) {
                Ok(payload) => {
                    let Some(cx) = BRIDGE_APP_HANDLE.get() else {
                        send_json_response(
                            request,
                            503,
                            json!({ "ok": false, "error": "Beam app context is not ready" }),
                        );
                        return;
                    };
...
pub fn start_bridge_server(cx: &BeamContext) {
    let _ = BRIDGE_APP_HANDLE.set(cx.clone());
