...
export function createEmptyRuntimeTreeSnapshot(): RuntimeTreeSnapshot {
  return {
    uiTree: new Map(),
    rootNodeId: null,
    toasts: [],
...
export function applyProtocolCommandsToRuntimeTree(
  ...
  const tempTree = new Map(snapshot.uiTree);
  const tempState = {
  ...
  for (const command of commands) {
    processCommand(command, propTemplates, tempTree, tempState, getMutableNode);
  }
