...
mod tests {
    use super::{
        detect_compositor_from, detect_desktop_environment_from, detect_session_type_from,
        DesktopEnvironmentKind, SessionType,
    };

    #[test]
    fn prefers_explicit_session_type() {
        let value = detect_session_type_from(Some("x11".to_string()), true, true);
        assert_eq!(value, SessionType::X11);
    }

    #[test]
    fn detects_hyprland_before_generic_wayland() {
        let session = SessionType::Wayland;
        let value = detect_desktop_environment_from(None, true, false, &session);
        assert_eq!(value, DesktopEnvironmentKind::Hyprland);
    }

    #[test]
    fn detects_gnome_from_current_desktop() {
        let session = SessionType::Wayland;
        let value = detect_desktop_environment_from(
            Some("ubuntu:GNOME".to_string()),
            false,
            false,
            &session,
        );
        assert_eq!(value, DesktopEnvironmentKind::Gnome);
    }
