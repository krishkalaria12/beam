...
impl Timezone {
    pub fn abbr(&self) -> String {
        ...
        let trunch_part = if trunch_part == 0 {
            String::new()
        } else if trunch_part > 0 {
            format!("+{}", trunch_part)
        } else {
            format!("{}", trunch_part)
        };

        let fract_part = if fract_part == 0 {
            String::new()
        } else {
...
pub fn get_current_timezone() -> Timezone {
    CURRENT_TIMEZONE.with(|current_timezone| current_timezone.borrow().clone())
}

pub static UTC_TIMEZONE: Lazy<Timezone> = Lazy::new(|| Timezone {
    offset: 0.0,
    name: "UTC".to_string(),
    isdst: false,
});

pub static TIMEZONE_LIST: Lazy<Vec<Timezone>> = Lazy::new(|| {
    let m = vec![
        Timezone {
            offset: 0.0,
