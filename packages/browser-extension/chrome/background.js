...
const FOCUS_BLOCK_URL = chrome.runtime.getURL("block.html");
...
async function enforceFocusPolicyForTab(tab, policy) {
  ...
  if (blockedOriginal) {
    ...
    if (!isBlockedByPolicy(blockedOriginal, policy)) {
      ...
      await chrome.tabs.update(tab.id, { url: blockedOriginal });
    }
    return;
  }
  ...
  await chrome.tabs.update(tab.id, { url: buildBlockUrl(tab.url, policy) });
}
...
function registerListeners() {
  ...
  chrome.tabs.onRemoved.addListener((tabId) => focusBlockedTabs.delete(tabId));
  chrome.tabs.onActivated.addListener((activeInfo) => {
    scheduleSync();
    void chrome.tabs.get(activeInfo.tabId).then(enforceLatestFocusPolicyForTab);
  });
