...
function shouldSkip(typeAlias) {
  ...
  const noExpandTag = comment.getTag?.("@noExpand") || comment.modifierTags?.has("@noExpand");
...
function findEnumReflectionByName(project, name) {
  function search(reflection) {
    ...
    if (reflection.kind === ReflectionKind.Enum && reflection.name === name) return reflection;
...
function buildEnumInfoFromReflection(enumReflection) {
  return {
    ...
    members: (enumReflection.children || []).map((child) => {
      const value =
        child.type?.value ?? child.defaultValue?.replace(/['"]/g, "") ?? child.name.toLowerCase();
