...
type ExtractRequestType<T extends RequestEndpoint> = T extends `${infer Category}.${infer Action}`
...
type ExtractResponseType<T extends ResponseEndpoint> = T extends `${infer Category}.${infer Action}`
...
class Bus {
  ...
  async request<T extends RequestEndpoint>(
    ...
    if (!resData) {
      return Err(
        Error(`Invalid response for request of type ${endpoint}: ${JSON.stringify(res, null, 2)}`),
      );
    }
