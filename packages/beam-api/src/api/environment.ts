...
export interface Environment {
  ...
  /**
   * The Beam version. Beam extensions should rely on this and ignore `raycastVersion`.
   */
  beamVersion: {
    tag: string;
    commit: string;
...
/**
 * General information about the running extension command, the beam version, the capabilities of
 * the system we are running on, etc...
 *
 * @example
 * ```typescript
 * import { environment } from '@beam-launcher/api';
 *
 * console.log({ environment });
 * ```
 */
export const environment: Environment = (globalThis as any).beam.environ;
