...
const GridRoot: React.FC<Grid.Props> = ({
  ...
  return (
    <grid fit={fit} inset={inset} aspectRatio={aspectRatioMap[aspectRatio]} {...props}>
      {searchBarAccessory}
      {children}
      {actions}
    </grid>
...
const GridItem: React.FC<Grid.Item.Props> = ({ detail, actions, content, accessory, ...props }) => {
  ...
  const tooltip = typeof content === "object" && "tooltip" in content ? content.tooltip : undefined;
  ...
  if (innerContent && typeof innerContent === "object" && "color" in innerContent) {
    ...
    serializedContent = innerContent ? serializeProtoImage(innerContent) : undefined;
  }
...
const GridSection: React.FC<Grid.Section.Props> = ({ fit, aspectRatio, inset, ...props }) => {
