...
import { type Color, type ColorLike, type SerializedColorLike, serializeColorLike } from "../color";
...
export declare namespace List {
  export type Props = {
    ...
    /**
     * Whether to use Beam's builtin fuzzy filtering.
     *
  ...
  export namespace Item {
    export type Props = {
       ...
       * If not explicitly specified, Beam will create one automatically.
       */
      id?: string;
    ...
    type Tag = AccessoryBase | { color: ColorLike; value: string | Date | undefined | null };
    type Text = AccessoryBase | { color: Color; value: string | Date | undefined | null };
...
export type SerializedText =
  | List.Item.AccessoryBase
  | { color: SerializedColorLike; value: string | Date | undefined | null };
export type SerializedAccessory = ({ tag?: SerializedTag } | { text?: SerializedText }) & {
  icon?: SerializedImageLike;
  tooltip?: string | null;
};

function serializeAccessory(accessory: List.Item.Accessory): SerializedAccessory {
  const icon = accessory.icon ? serializeProtoImage(accessory.icon) : undefined;

  const tag =
    typeof accessory === "object" && "tag" in accessory ? serializeTag(accessory.tag) : undefined;
  const text =
...
const ListRoot: React.FC<List.Props> = ({ searchBarAccessory, children, actions, ...props }) => {
...
const ListItem: React.FC<List.Item.Props> = ({ detail, actions, icon, accessories, ...props }) => {
  ...
  return (
    <list-item {...props} icon={serializedIcon} accessories={serializedAccessories} id={id.current}>
      {detail}
      {actions}
    </list-item>
  );
...
const ListItemDetail: React.FC<List.Item.Detail.Props> = ({ metadata, ...props }) => {
