...
const ActionPanelSection: React.FC<React.PropsWithChildren<ActionPanel.Section.Props>> = (
  props,
) => {
  ...
  return <action-panel-section {...nativeProps}>{props.children}</action-panel-section>;
};

const ActionPannelSubmenu: React.FC<ActionPanel.Submenu.Props> = ({ children, icon, ...props }) => {
  const stableIdRef = useRef<string | undefined>(undefined);
  if (!stableIdRef.current) {
    stableIdRef.current = randomUUID();
  }
  ...
  return (
    <action-panel-submenu {...props} icon={serializedIcon} stableId={stableIdRef.current}>
      {children}
