...
import { type Application, open, runInTerminal, showInFileBrowser } from "../utils";
...
const ActionRoot: React.FC<ActionProps> = ({ icon, ...props }) => {
  ...
  return <action {...props} icon={serializedIcon} stableId={stableIdRef.current} />;
};
...
const SubmitForm: React.FC<Action.SubmitForm.Props> = ({ title = "Submit", ...props }) => {
  const stableIdRef = useRef<string | undefined>(undefined);
  if (!stableIdRef.current) {
    stableIdRef.current = randomUUID();
  }
