...
const preferenceSchema = z.discriminatedUnion("type", [
  ...
  z
    .object({
      ...
      label: z.string().min(1).describe("The label of the checkbox. Shown next to the checkbox."),
      default: z
        .union([
          z.boolean(),
          z.object({
...
const argumentBase = {
  ...
  placeholder: z.string().min(1).describe("Placeholder for the argument's input field."),
  required: z
    .boolean()
    .describe(
...
export default z.object({
  ...
  debug: z
    .object({
      reloadShortcut: z
        ...
        .strict()
        .describe(
          "Keyboard shortcut used to refresh/reload a command while the Beam window is focused.",
        )
        .optional(),
    })
      ...
      z
        .object({
          icon: z
            ...
            .regex(new RegExp("^[^\\s]+(?:[ ]*[^\\s]+)*$"))
            .describe(
              "A 512x512 icon representing the command. It will be displayed in Preferences and Beam root search. If a command doesn't have an icon, it will 'inherit' the extension's icon. Please note that light and dark themes are supported; just append '@dark' or '@light' to the icons name and the correct icon will be picked at run-time. For example, write for this property 'icon.png' and have in the assets folder the 'icon@light.png' and 'icon@dark.png' assets.",
            )
            .optional(),
          mode: z
            .enum(["view", "no-view", "menu-bar"])
            .describe(
              "A value of 'view' indicates that the command will show a main view when performed. 'no-view' means that the command does not push a view to the main navigation stack in Beam. The latter is handy for directly opening URL or other API functionality that doesn't require a user interface. 'menu-bar' renders an extra item in the macOS system menu bar at the top of the screen.",
            ),
          name: z
            .string()
          ...
          title: z
            ...
            .max(255)
            .describe(
              "The command title will be displayed in the Store, Preferences, and in Beam's root search.",
            ),
          ...
          arguments: z
            ...
            .optional(),
          disabledByDefault: z.boolean().describe("Defaults to `false`").optional(),
        })
        .catchall(z.any()),
    )
    .min(1)
      ...
      z
        .object({
          icon: z
            ...
            .regex(new RegExp("^[^\\s]+(?:[ ]*[^\\s]+)*$"))
            .describe(
              "A 512x512 icon representing the tool. It will be displayed in various places across Beam. If a tool doesn't have an icon, it will 'inherit' the extension's icon. Please note that light and dark themes are supported; just append '@dark' or '@light' to the icons name and the correct icon will be picked at run-time. For example, write for this property 'icon.png' and have in the assets folder the 'icon@light.png' and 'icon@dark.png' assets.",
            )
            .optional(),
          name: z
            .string()
          ...
          title: z
            ...
            .max(255)
            .describe(
              "The tool title will be displayed in the Store, Preferences, and any other places the tool might be referenced in the Beam UI.",
            ),
          keywords: z.any().optional(),
          description: z
            .string()
      ...
      evals: z
        .array(
          z.object({
            ...
            usedAsExample: z
              .boolean()
              .describe("Whether the eval can be used as an example in Beam (default `true`)")
              .optional(),
          }),
        )
  ...
  preferences: z
    .array(preferenceSchema)
    .describe(
      "Extensions can contribute preferences that are shown in Beam Preferences > Extensions. You can use preferences for configuration values and passwords or personal access tokens.",
    )
    .optional(),
  categories: z
    .array(
    ...
    .optional(),
  contributors: z
    .array(z.string().regex(new RegExp("^[a-zA-Z0-9-*~][a-zA-Z0-9-*._~]*$")).min(2).max(75))
    .min(0)
