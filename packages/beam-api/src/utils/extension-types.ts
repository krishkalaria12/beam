 ...
 */
export function updateExtensionTypes(manifest: Manifest, outDir: string): { envPath: string } {
  const content = createExtensionTypes(manifest);
  const envPath = join(outDir, "beam-env.d.ts");
...
function getPreferenceType(pref: Preference): string {
  ...
  if (pref.type === "dropdown" && Array.isArray(pref.data) && pref.data.length > 0) {
...
function getArgumentType(arg: Argument): string {
  if (arg.type === "dropdown" && Array.isArray(arg.data) && arg.data.length > 0) {
