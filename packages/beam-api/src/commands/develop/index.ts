...
import { extensionDataDir, extensionInternalSupportDir } from "../../utils/utils.js";
import { updateExtensionTypes } from "../../utils/extension-types.js";
import { BeamClient } from "../../utils/beam.js";
...
export default class Develop extends Command {
  ...
  static examples = [`<%= config.bin %> <%= command.id %> --target /path/to/extension`];
  static flags = {
    target: Flags.string({
      aliases: ["input"],
      char: "i",
  ...
  async run(): Promise<void> {
    ...
    const typeCheck = async (): Promise<TypeCheckResult> => {
      const spawned = spawn("bunx", ["tsc", "--noEmit"]);
	  ...
	  */
      ...
      const promises = manifest.commands.map((cmd) => {
        ...
        return esbuild.build({
          bundle: true,
          entryPoints: [source],
          external: ["react", "@beam-launcher/api", "@raycast/api"],
          format: "cjs",
          outdir: outDir,
          platform: "node",
          loader: {
    ...
    const pingError = beam.ping();

    if (pingError) {
      console.error(`Failed to ping beam\n`, pingError.message);
      return;
    }
    ...
    await Promise.all(
      [extensionDir, internalSupportDir].map((dir) => fsp.mkdir(dir, { recursive: true })),
    );
    ...
    process.on("SIGINT", () => {
      ...
      beam.stopDevSession(id);
      throw new Error(`Development session interrupted`);
    });

    const error = beam.startDevSession(id);

    if (error) {
      console.error(`Failed to invoke beam`, error);
      return;
    }
