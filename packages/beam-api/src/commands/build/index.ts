...
export default class Build extends Command {
  static args = {};
  static description = "Build an extension bundle";
  static examples = [`<%= config.bin %> <%= command.id %> --src /path/to/extension`];
  static flags = {
    out: Flags.string({
      aliases: ["output"],
      char: "o",
      description:
        "Path to output the compiled extension bundle to. Defaults to Beam extension directory.",
      required: false,
    }),
    src: Flags.string({
      aliases: ["src"],
  ...
  async run(): Promise<void> {
    ...
    const build = async (outDir: string) => {
      const entryPoints = manifest.commands.map((cmd) => join("src", `${cmd.name}.tsx`));
    ...
    const typeCheck = spawnSync("bunx", ["tsc", "--noEmit"]);
