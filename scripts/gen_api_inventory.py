...
lines.append("")
lines.append("Note on the plan's figure: the plan says '87 IPC commands'; the tree at")
lines.append("main @ 9d830ba registers **162** shared-list commands plus **4**")
lines.append("platform-extra commands (the macro's $extra mechanism) = **166** total.")
lines.append("This list is normative (rule R9). Regenerate with")
lines.append("`python3 scripts/gen_api_inventory.py`.")
lines.append("")
lines.append("## Status: the frozen surface is fully accounted for")
lines.append("")
lines.append("- **154 ported** - every command exists as a service function with the")
lines.append("  same name in `crates/beam-services`.")
lines.append("- **8 deleted (D5)** - the theming surface: get/set_ui_style,")
lines.append("  get/set_base_color, and the four launcher_theme commands. The store")
lines.append("  keys are ignored; the CHANGELOG announces the loss at cutover.")
lines.append("- **4 absorbed (SD-1)** - the resize-transition IPC dance; the beam")
lines.append("  window module owns sizing directly.")
lines.append("- **0 pending**.")
lines.append("")
lines.append("Total: %d commands; %d ported." % (len(unique), len(ported & set(names))))
lines.append("")
